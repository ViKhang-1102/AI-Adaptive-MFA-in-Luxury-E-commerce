        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm divide-y divide-neutral-50">
                    @forelse($customerMonthlyRevenue as $revenue)
                    <tr class="hover:bg-neutral-50 transition-colors">
                        <td class="py-4 pr-4 font-bold text-neutral-700">{{ $revenue->name ?? 'Unknown' }}</td>
                        <td class="py-4 px-4 text-neutral-500">{{ $revenue->ym }}</td>
                        <td class="py-4 px-4 text-right font-bold text-green-600">${{ number_format($revenue->total_amount, 2) }}</td>
                        <td class="py-4 px-4 text-right text-blue-600">${{ number_format($revenue->seller_profit, 2) }}</td>
                        <td class="py-4 px-4 text-right text-neutral-600">{{ $revenue->order_count }}</td>
                        <td class="py-4 pl-4 text-right">
                            @if($revenue->trust_score >= 80)
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-50 text-green-700 border border-green-200">{{ $revenue->trust_score }}</span>
                            @elseif($revenue->trust_score >= 60)
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-700 border border-amber-200">{{ $revenue->trust_score }}</span>
                            @else
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-50 text-red-700 border border-red-200">{{ $revenue->trust_score }}</span>
                            @endif
                        </td>
                    </tr>
                    @empty
