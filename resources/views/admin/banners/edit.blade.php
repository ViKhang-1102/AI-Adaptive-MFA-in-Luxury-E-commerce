...
<div class="max-w-2xl mx-auto px-4 py-8">
    <a href="{{ route('admin.banners.index') }}" class="text-blue-600 hover:underline mb-4 inline-block">← Back</a>
        ...
        <form action="{{ route('admin.banners.update', $banner) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" value="{{ old('title', $banner->title) }}"
                    class="w-full px-4 py-2 border rounded @error('title') border-red-500 @enderror" required>
                @error('title')
            ...
            <div class="mb-4">
                ...
                @if($banner->image)
                    <img src="{{ asset('storage/' . $banner->image) }}" alt="{{ $banner->title }}" class="h-32 w-48 object-cover rounded mb-2">
                @endif
                <label class="block font-bold mb-2">Change Image (Optional)</label>
                <input type="file" name="image" accept="image/*"
                    class="w-full px-4 py-2 border rounded @error('image') border-red-500 @enderror">
                @error('image')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('admin.banners.index') }}" class="px-6 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 font-semibold">
                    Cancel
                </a>
            </div>
        </form>
