...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="mb-6">
        <a href="{{ route('seller.dashboard') }}" class="inline-flex items-center text-sm font-medium text-neutral-500 hover:text-primary transition-colors group">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
        <div>
            <h1 class="text-3xl font-serif font-bold text-primary mt-0">My Wallet</h1>
            <p class="text-neutral-500 mt-1">Manage your earnings and request withdrawals.</p>
        </div>

        <form method="GET" action="{{ route('seller.wallet') }}" class="flex gap-2 items-center bg-white p-2 rounded-xl shadow-sm border border-neutral-100">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div class="bg-primary p-6 rounded-2xl shadow-hover flex flex-col relative overflow-hidden group border border-primary-light/20">
            ...
            <div class="relative z-10 flex-1">
                ...
                <h2 class="text-4xl font-bold text-white mb-2">${{ number_format($totalGrossRevenue ?? 0, 2) }}</h2>
        ...
        <div class="bg-orange-50 p-6 rounded-2xl shadow-soft border border-orange-100 flex flex-col group hover:border-orange-200 transition-colors relative overflow-hidden">
            ...
            <div class="relative z-10 flex-1">
                ...
                <h2 class="text-4xl font-bold text-orange-600 mb-2">${{ number_format($pendingBalance, 2) }}</h2>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-soft border border-neutral-100 flex flex-col group hover:border-gold/50 transition-colors">
            ...
            <h2 class="text-3xl font-bold text-primary mb-2">${{ number_format($totalWithdrawn ?? 0, 2) }}</h2>
                            ...
                            <tbody class="divide-y divide-neutral-100">
                                ...
                                <tr class="hover:bg-neutral-50/50 transition-colors group">
                                    <td class="px-6 py-4 text-neutral-500 whitespace-nowrap">
                                        ...
                                        <span class="text-xs text-neutral-400">{{ $transaction->created_at->format('H:i') }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 
                                                {{ $transaction->type === 'credit' ? 'bg-green-50 text-green-500' : 'bg-red-50 text-red-500' }}">
                                                <i data-lucide="{{ $transaction->type === 'credit' ? 'arrow-down-left' : 'arrow-up-right' }}" class="w-4 h-4"></i>
                                            </div>
                                            <div>
                                                <p class="font-medium text-primary">{{ $transaction->description }}</p>
                                                <p class="text-xs text-neutral-500">{{ ucfirst($transaction->type) }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border {{ $statusClass }}">
                                            {{ $transaction->status }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-right whitespace-nowrap">
                                        <span class="font-bold {{ $transaction->type === 'credit' ? 'text-green-600' : 'text-primary' }}">
                ...
                <form action="{{ route('seller.wallet.withdraw') }}" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="amount" step="0.01" max="{{ $actualBalance }}" placeholder="0.00" 
                        ...
                        <div class="flex justify-between items-center mt-2">
                            <span class="text-xs text-neutral-500">Max available: <span class="font-bold text-primary">${{ number_format($actualBalance, 2) }}</span></span>
                            <button type="button" onclick="document.querySelector('input[name=amount]').value = {{ $actualBalance }}" class="text-xs font-medium text-gold-dark hover:text-gold transition-colors">Max</button>
                        </div>
                    </div>
