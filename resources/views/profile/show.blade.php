...
<div class="max-w-7xl mx-auto px-4 py-8">
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('home') }}" class="px-4 py-2 bg-neutral-500 text-white rounded-md hover:bg-gray-600 font-semibold">
            ← Home
        </a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-md-lg shadow-sm h-fit">
            ...
            <div class="space-y-2">
                ...
                <a href="{{ route('addresses.index') }}" class="block px-4 py-2 rounded-md hover:bg-neutral-100">
                    My Addresses
                </a>
                @endif
            </div>
            ...
            <div id="info-section" class="bg-white p-6 rounded-md-lg shadow-sm">
                ...
                <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        ...
                        @if(auth()->user()->avatar)
                        <img src="{{ asset('storage/' . auth()->user()->avatar) }}" alt="Avatar" class="w-24 h-24 rounded-md mb-2 object-cover" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%2224%22 height=%2224%22%3E%3Ccircle cx=%2212%22 cy=%2212%22 r=%2212%22 fill=%22%23ddd%22/%3E%3C/svg%3E'">
                        @else
                    ...
                    <div>
                        ...
                        <textarea name="address" rows="3" 
                            class="w-full px-4 py-2 border rounded-md">{{ auth()->user()->address }}</textarea>
                    </div>

                    <div>
                        <label class="block font-bold mb-2">Bio</label>
                        <textarea name="bio" rows="3" maxlength="500"
                            class="w-full px-4 py-2 border rounded-md">{{ auth()->user()->bio }}</textarea>
                    </div>
