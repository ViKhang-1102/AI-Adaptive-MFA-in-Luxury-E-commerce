...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 h-screen">
        ...
        <div class="lg:col-span-1 bg-white rounded-md-lg shadow-sm overflow-hidden flex flex-col">
            <div class="p-4 border-b bg-neutral-50">
                <h2 class="font-bold text-lg">Customers</h2>
                <p class="text-sm text-neutral-600">Click to view messages</p>
            </div>
        ...
        <div class="lg:col-span-3 bg-white rounded-md-lg shadow-sm overflow-hidden flex flex-col">
            ...
            <div id="conversation-panel" class="hidden h-full overflow-hidden flex flex-col">
                <div id="conv-header" class="p-4 border-b bg-neutral-50">
                    <button id="back-to-products" class="text-primary hover:underline mb-2">&larr; Back to Products</button>
                    <h2 id="conv-product-name" class="font-bold text-lg">Loading...</h2>
                    <p id="conv-customer-name" class="text-sm text-neutral-600">Loading...</p>
                </div>
                
                <div id="messages-container" class="flex-1 overflow-y-auto p-4 bg-neutral-100">
...
<script>
    const sellerId = <?php echo e(auth()->id()); ?>;
    ...
    async function loadCustomers() {
        try {
            ...
            if (customers.length === 0) {
                list.innerHTML = '<p class="p-4 text-neutral-600 text-sm">No customers yet</p>';
                return;
            }
            ...
            customers.forEach(customer => {
                ...
                const badge = customer.unread_count > 0 
                    ? `<span class="ml-2 bg-red-600 text-white text-xs px-2 py-1 rounded-md-full">${customer.unread_count}</span>`
                    : '';
                ...
                div.innerHTML = `
                    <div class="flex justify-between items-center">
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-neutral-600 truncate">${escapeHtml(customer.last_message)}</p>
    ...
    async function loadProducts(customerId) {
        try {
            ...
            if (products.length === 0) {
                panel.innerHTML = '<div class="p-8 text-center text-neutral-500"><p>No products with messages from this customer</p></div>';
                return;
            }

            const title = document.createElement('div');
            title.className = 'p-4 border-b bg-neutral-50 sticky top-0';
            ...
            products.forEach(product => {
                ...
                const badge = product.unread_count > 0 
                    ? `<span class="ml-2 bg-red-600 text-white text-xs px-2 py-1 rounded-md-full">${product.unread_count}</span>`
                    : '';

                div.innerHTML = `
                    <div class="flex justify-between items-center">
                        <div class="flex-1 flex items-center gap-3">
                            ${product.image ? `<img src="${product.image}" class="w-10 h-10 object-cover rounded-md">` : '<div class="w-10 h-10 bg-gray-300 rounded-md"></div>'}
                            <div class="flex-1">
                                <p class="font-semibold text-sm">${escapeHtml(product.name)}</p>
                                <p class="text-xs text-neutral-600 truncate">${escapeHtml(product.last_message)}</p>
    ...
    async function loadMessages(productId, customerId) {
        try {
            ...
            if (messages.length === 0) {
                container.innerHTML = '<p class="text-center text-neutral-600 py-8">No messages yet. Start the conversation!</p>';
                return;
            }
            ...
            messages.forEach(msg => {
                ...
                div.innerHTML = `
                    <div class="${isOwn ? 'bg-blue-500 text-white' : 'bg-gray-300 text-black'} rounded-md-lg px-3 py-2 inline-block max-w-xs break-words">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\E-commerce2026\resources\views/seller/messages/index_new.blade.php ENDPATH**/ ?>