                ...
                if (in_array($order->status, ['pending', 'review', 'confirmed', 'paid', 'processing'])) {
                    ...
                    if ($order->seller) {
                        \App\Models\OrderNotification::create([
                            ...
                            'message' => "Order #{$order->id} has been cancelled because the customer account was deleted by administration.",
                        ]);
                        ...
                        if ($firstItem && $firstItem->product_id) {
                            \App\Models\Message::create([
                                ...
                                'message' => "Order #{$order->id} from {$user->name} was cancelled as the user account was removed.",
                            ]);
                        }
                    }
                }
